...
export function reciprocalRankFusion(results: Document[][], k = 60) {
    ...
    const reranked = Array.from(fusedScores.entries()).map(([key, score]) => {
        // Retrieve original document from one of the arrays
        const doc = results.flat().find(d => d.pageContent === key);
        return { doc, score };
    }).sort((a, b) => b.score - a.score);

    // Return the top 10 reranked documents
    return reranked.slice(0, 10).map(item => item.doc).filter(doc => doc !== undefined);
}