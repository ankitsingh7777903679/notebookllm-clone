...
export const generateImage = async (prompt: string, path: string, fileName: string, cb: (fileName: string) => void) => {
    ...
    if (!API_KEY) {
        console.warn("FIREWORKS_API_KEY is missing. Writing placeholder image.");
        // write a 1x1 transparent PNG placeholder so the saved note image URL points to a valid image
        ...
        try {
            await fs.promises.writeFile(`${path}/${fileName}.png`, placeholderBuffer);
            cb(`${path}/${fileName}.png`);
            console.log('Wrote placeholder image as png');
        } catch (err) {
            console.error('Failed to write placeholder image', err);
        }
        return;
    }
    ...
    if (!buffer || buffer.length === 0) {
        console.warn('Image generation returned empty buffer, writing placeholder image');
        const placeholderBase64 = "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR4nGMAAQAABQABDQottAAAAABJRU5ErkJggg==";
        const placeholderBuffer = Buffer.from(placeholderBase64, 'base64');
        try {
            await fs.promises.writeFile(`${path}/${fileName}.png`, placeholderBuffer);
    ...
    fs.writeFile(`${path}/${fileName}.png`, buffer, (err) => {
        if (err) {
            console.error('Failed to write generated image', err);
            return;
        }
        cb(`${path}/${fileName}.png`)
        console.log('Image saved as png, bytes:', buffer.length);
    });
    return;
